...
function render(data) {
  ...
  $('product_data_source').value = data.capabilities?.product?.source ?? 'mock';
  $('pricing_capability_enabled').checked = Boolean(data.capabilities?.pricing?.enabled ?? true);
  $('pricing_data_source').value = data.capabilities?.pricing?.source ?? 'mock';
  $('knowledge_capability_enabled').checked = Boolean(data.capabilities?.knowledge?.enabled ?? true);
  $('knowledge_data_source').value = data.capabilities?.knowledge?.source ?? 'mock';
...
form.addEventListener('submit', async (event) => {
  ...
  const response = await fetch('/api/v1/admin/config', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({
    ...
    product_data_source: $('product_data_source').value, pricing_capability_enabled: $('pricing_capability_enabled').checked, pricing_data_source: $('pricing_data_source').value,
    knowledge_capability_enabled: $('knowledge_capability_enabled').checked, knowledge_data_source: $('knowledge_data_source').value,
    vision_capability_enabled: $('vision_capability_enabled').checked, vision_data_source: $('vision_data_source').value
  })});
